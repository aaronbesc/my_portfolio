...
export default function Home() {
  ...
  const words = ["Computer Science major", "Electrical Engineering minor", "senior UF student"];
  ...
  useEffect(() => {
    ...
    const typeEffect = () => {
      ...
      if (isDeleting) {
        ...
        setDisplayText((prev) => currentWord.slice(0, j + 1));
        j++;
        if (j === currentWord.length) {
          isDeleting = true;
          setTimeout(typeEffect, 1500); // Delay before deleting
          return;
        }
      }
      setTimeout(typeEffect, isDeleting ? 100 : 50);
    };
  ...
  return (
    <>
      ...
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 175"><path fill="#fff" fill-opacity="1" d="M0,96L60,80C120,64,240,32,360,48C480,64,600,128,720,138.7C840,149,960,107,1080,85.3C1200,64,1320,64,1380,64L1440,64L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z"></path></svg>
      </div>
      ...
      <div className="container mx-auto p-6">
        ...
        {/* Projects Section */}
        <section>
          <div className="grid grid-cols-4 border border-[0.5px] border-black">
            {/* First Row: Project #1, Project #2, etc. */}
            {projects.map((project, index) => (
              <div key={project.id} className="border border-black p-3 flex items-center justify-between">
                .0{index + 1}
                <a href={project.githubLink}><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-github"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg></a>
              </div>
            ))}

            {/* Second Row: Project Details */}
            {projects.map((project) => (
              <div key={project.id} className="border border-black p-10">
                {/* Category Label */}
        ...
        <section className="text-center mt-10 p-6 border rounded-lg shadow-md">
          ...
          <p className="text-gray-600 mt-2">Feel free to reach out for collaboration or project inquiries.</p>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 150">
          <path fill="#fff" fill-opacity="1" d="M0,96L48,101.3C96,107,192,117,288,128C384,139,480,149,576,149.3C672,149,768,139,864,122.7C960,107,1056,85,1152,74.7C1248,64,1344,64,1392,64L1440,64L1440,0L1392,0C1344,0,1248,0,1152,0C1056,0,960,0,864,0C768,0,672,0,576,0C480,0,384,0,288,0C192,0,96,0,48,0L0,0Z"></path>
        </svg>
        </div>
